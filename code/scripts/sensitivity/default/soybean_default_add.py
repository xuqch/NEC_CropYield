...
print("Syear: %s" % Syear)
print("Eyear: %s" % Eyear)
fname = os.path.join('/stu01/xuqch3/PCSE/NEX-GDDP-CMIP6/', "output/sensitivity/default_add/")
...
def FilePrapare(metrodata_dir, ssp):
    ...
    with xr.open_dataset(VarFile, decode_times=True, drop_variables=['height_2', 'height'],
        ...
        # pr = ds.pr.mean()
        ds['pr'] = ds['pr'] + 10./86400#pr.values
        try:
            print(" ")
            print("**********************")
            print(" ")
...
if __name__ == '__main__':
    ...
    for ssp in ssps:
        ...
        ds = FilePrapare(f'/stu01/xuqch3/PCSE/NEX-GDDP-CMIP6/', ssp)#控制输入文件地址
