...
print("Syear: %s" %  Syear)
print("Eyear: %s" %  Eyear)
fname = os.path.join(data_dir, "output")
...
def FilePrapare(metrodata_dir):
    ...
    with xr.open_dataset(VarFile, decode_times=True,drop_variables=['height_2','height'],chunks={'time': 3650}) as ds:
        try:
            print(" ")
            print("**********************")
            print(" ")
...
def Initial(data_dir, Start_time):
    # print(ds)
    ...
    sited = WOFOST71SiteDataProvider(WAV=10, CO2=360., SMLIM=0.35)
    ...
    for ilon in enumerate(ds.lon.values):
        ...
        for ilat in enumerate(ds.lat.values):
            ...
            if da.tasmax.values[0]>-9999.0:
                # print("--Continue location: lon-->%s, lat-->%s" % (ilon, ilat))
                ...
                for i, inputs in enumerate(all_runs):
                    ...
                    # parameters = ParameterProvider(sitedata=sited, soildata=soild, cropdata=crop_data)
                    # Start WOFOST, run the simulation
                    try:
                        wofost = Wofost71_PP(parameters, wdp, agromanagement)
...
def core_cal(ds, ilonn, ilatn, parameters,temp,Start_time):
...
def cal_run(ds,parameters,temp,Start_time,dk):
    ...
    for ilonn in range(len(ds.lon.values)):
        ...
        n=n+1
        print("now at location: lon-->%s, %s" %(ds.lon.values[ilonn],n))
        temp0 = Parallel(n_jobs=num_cores)(delayed(core_cal)(ds,ilonn,i,parameters,temp,Start_time) for i in range(len(ds.lat.values)))
        for ilatn in range(len(ds.lat.values)):
            temp1 = temp0[ilatn]
            # print(test['lat'])
            if temp1['lat'] > 0:
...
if __name__ == '__main__':
    ...
    for ssp in ssps:
        ...
        ds=FilePrapare('/tera01/xuqch3/PCSE/PCSE_input/%s'%(ssp))
        Start_time = xr.open_dataset('/tera01/xuqch3/Dongbei/acctas/Growthday_ensmean_%s.nc'%(ssp)).ta10
        # ds = FilePrapare('/Volumes/xuqch_min/PCSE/pcse')
        # Start_time = xr.open_dataset('/Volumes/xuqch_min/PCSE/pcse/Growthday_ensmean_ssp126.nc').ta10
        ds = ds.sel(time=slice(Sdate, Edate))
        parameters,temp=Initial(data_dir, Start_time)
        ...
        cal_run(ds,parameters,temp,Start_time,dk)
        print(dk)
        dk.to_netcdf('%s/rice_output_%s.nc' % (fname,ssp), engine='netcdf4')
