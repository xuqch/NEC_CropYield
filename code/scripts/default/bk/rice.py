...
co2data_dir=os.path.join(os.getcwd(), "../data/co2/")
...
def Initial(data_dir, Start_time,dco2):
    # print(ds)
        ...
        for ilat in enumerate(ds.lat.values):
            ...
            if da.tasmax.values[0]>-9999.0:
                ...
                for i, inputs in enumerate(all_runs):
                    ...
                    print(dco2.loc[year].values)

                    sited = WOFOST72SiteDataProvider(WAV=10, CO2=dco2.loc[year].values, SMLIM=0.35)
...
def cal_run(ds,temp,Start_time,dk,dco2):
    ...
    for ilonn in range(len(ds.lon.values)):
        ...
        temp0 = Parallel(n_jobs=num_cores)(delayed(core_cal)(ds,ilonn,i,temp,Start_time,dco2) for i in range(len(ds.lat.values)))
        for ilatn in range(len(ds.lat.values)):
            temp1 = temp0[ilatn]
            # print(test['lat'])
            if temp1['lat'] > 0:
...
if __name__ == '__main__':
    ...
    for ssp in ssps:
        ...
        temp=Initial(data_dir, Start_time,dco2)
