...
def plot_trend(ds,path,var,ssp):
    ...
    cbar=plt.colorbar(shrink = 1.0, spacing='uniform',orientation='horizontal',label='Annual trend',fraction=0.046, pad=0.04)
...
if __name__=='__main__':
    ...
    models=['ACCESS-CM2','ASSESS-ESM1-5','BCC_CSM2_MR','CanESM5']
    #models=['BCC_CSM2_MR','CanESM5']
    for model in models:
        scratch           = ('/Volumes/SSD-2/Dongbei/Processed/%s/scratch/'%(model))
        pathout_Dongbei   = ('/Volumes/SSD-2/Dongbei/Processed/%s/trend'%(model))
            ...
            for Var in Vars:
                ...
                for SubVar in glob.glob(("%s_*.nc")%(Var)):
                    ...
                    with xr.open_dataset(VarFile) as ds:
                        ds = ds.pipe(lambda x: x).where(ds > -3000.0, drop=True)
                        ds=ds['%s'%(Var)]*86400.0*365.0
                        ds_annual=ds.resample(time='1Y').mean()
                        #ds_annual=ds.resample(time='1Y').sum()
