    ...
    for scenario in scenarios:
        for name, idx, colr in zip(names, idxs, colors):
            for ssp in ssps:
                ...
                lons = xr.open_dataset(f'{path}/PCSE/pr_t/pr_t_Growthday_ssp585.nc').lon.values
                lats = xr.open_dataset('{path}/PCSE/pr_t/pr_t_Growthday_ssp585.nc').lat.values
                # fixtime=Start_time*0+105
                ds1 = xr.open_dataset(f'{path}/PCSE/output/sensitivity/default/{name}_output_{ssp}_default.nc')
                ...
                Max_Yield_default.to_netcdf(f'{pathin}/{name}_{run_name}_output_{ssp}_sowing_Max_Yield_default.nc')
                Max_Yield = Max_Yield_default  # ds4.groupby("time.year").max("time")

                ds5 = xr.open_dataset(f'{path}/PCSE/pr_t/pr_t_Growthday_{ssp}.nc')
                ...
                for i in np.arange(0, 77, 7):
                    VarFile = f'{pathin}/{name}_output_{ssp}_sowing_{i}.nc'
                    ...
                    Max_Yield_i.to_netcdf(f'{path}/PCSE/output/adaptation/sowing/{name}_{run_name}_output_{ssp}_sowing_Max_Yield_{i}.nc')
                ...
                Max_Yield.to_netcdf(f'{path}/PCSE/output/adaptation/sowing/{name}_{run_name}_output_{ssp}_sowing_Max_Yield_final.nc')
                Start_time1.to_netcdf(f'{path}/PCSE/output/adaptation/sowing/{name}_{run_name}_output_{ssp}_sowing_Growthday_final.nc')
